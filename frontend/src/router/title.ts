...
export function resolveDocumentTitle(routeTitle: unknown, siteName?: string, titleKey?: string): string {
  const configuredSiteName = typeof siteName === 'string' ? siteName.trim() : ''
  const normalizedSiteName = !configuredSiteName || configuredSiteName === 'Sub2API' ? 'CheapAPI' : configuredSiteName
