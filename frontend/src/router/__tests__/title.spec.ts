...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard - CheapAPI')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('CheapAPI')
    expect(resolveDocumentTitle('Dashboard', 'Sub2API')).toBe('Dashboard - CheapAPI')
  })
